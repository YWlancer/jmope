...
public class BTree {
	...
	class Node {
		...
		// Remove an element from a node and also the left (0) or right (+1)
		// child.
	...
	void splitChildNode(Node parentNode, int i, Node node) {
		...
		for (int j = 0; j < T - 1; j++) { // Copy the last T-1 elements of node
											// into newNode.
			newNode.mKeys[j] = node.mKeys[j + T];
			newNode.mObjects[j] = node.mObjects[j + T];
		}
		if (!newNode.mIsLeafNode) {
			for (int j = 0; j < T; j++) { // Copy the last T pointers of node
											// into newNode.
			...
			if ((i = node.binarySearch(key)) != -1) { 
				...
				if (leftChildNode.mNumKeys >= T) { 
					...
					while (!predecessorNode.mIsLeafNode) { 
						// Therefore only descend to the previous node (erasureNode) of the predecessor node and delete the key using 3.
					...
					delete(erasureNode, node.mKeys[i]);
				} else { // 2c. If both the predecessor and the successor child
							// node have only T - 1 keys...
				// If both of the two child nodes to the left and right of the
				// deleted element have the minimum number of elements,
				// namely T - 1, they can then be joined into a single node with
				// 2 * T - 2 elements.
					int medianKeyIndex = mergeNodes(leftChildNode,
							rightChildNode);
					moveKey(node, i, RIGHT_CHILD_NODE, leftChildNode,
							medianKeyIndex); // Delete i's right child pointer
												// from node.
					delete(leftChildNode, key);
				}
			} else { // 3. If the key is not resent in node, descent to the root
						// of the appropriate subtree that must contain key...
			// The method is structured to guarantee that whenever delete is
			// called recursively on node "node", the number of keys
			// in node is at least the minimum degree T. Note that this
			// condition requires one more key than the minimum required
			// by usual B-tree conditions. This strengthened condition allows us
			// to delete a key from the tree in one downward pass
			// without having to "back up".
				i = node.subtreeRootNodeIndex(key);
				Node childNode = node.mChildNodes[i]; // childNode is i-th child
														// of node.﻿ ﻿ ﻿ ﻿
				if (childNode.mNumKeys == T - 1) {
					Node leftChildSibling = (i - 1 >= 0) ? node.mChildNodes[i - 1]
							: null;
					...
					if (leftChildSibling != null && leftChildSibling.mNumKeys >= T) { 
						...
						// Move a key from the subtree's root node down into
						// childNode along with the appropriate child pointer.
						// Therefore, first shift all elements and children of
						// childNode right by 1.
						childNode.shiftRightByOne();
						// i - 1 is the key index in node that is smaller than childNode's smallest key.
						...
						// Move a key from the left sibling into the subtree's
						// root node.
						...
						// Remove the key from the left sibling along with its
						// right child node.
						leftChildSibling.remove(leftChildSibling.mNumKeys - 1,
								RIGHT_CHILD_NODE);
					} else if (rightChildSibling != null && rightChildSibling.mNumKeys >= T) { 
						// 3a. The right sibling has >= T keys... ﻿ ﻿ ﻿ ﻿
						// Move a key from the subtree's root node down into
						// childNode along with the appropriate child pointer.
						...
						if (leftChildSibling != null) {
							...
							// i - 1 is the median key index in node when merging with the left sibling.
							...
							// i is the median key index in node when merging with the right sibling.
	...
	int mergeNodes(Node dstNode, Node srcNode) {
		...
		if (srcNode.mKeys[0] < dstNode.mKeys[dstNode.mNumKeys - 1]) {
			int i;
			// Shift all elements of dstNode right by srcNode.mNumKeys + 1 to
			// make place for the srcNode and the median key.
	...
	/*public List<Object> searchCollection(int key, String token) {
	...
	String printBTree(Node node, String indent) {
		String string = "";
		if (node != null) {
			if (node.mIsLeafNode) {
				for (int i = 0; i < node.mNumKeys; i++) {
					string += indent + node.mObjects[i] + "\n";
				}
			} else {
				int i;
				for (i = 0; i < node.mNumKeys; i++) {
					string += indent + printBTree(node.mChildNodes[i], indent + "     ");
					string += indent + node.mObjects[i] + "\n";
				}
				string += indent + printBTree(node.mChildNodes[i], indent + "     ");
			}
	...
	public static void main(String[] args) {
		...
		System.out.println(bt.search2(2000));
	}
}